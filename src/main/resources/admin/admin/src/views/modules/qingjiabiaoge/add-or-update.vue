<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.qingjiabiao" label="请假表" prop="qingjiabiao">
          <file-upload
          tip="点击上传请假表"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.qingjiabiao?ruleForm.qingjiabiao:''"
          @change="qingjiabiaoUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.qingjiabiao" label="请假表" prop="qingjiabiao">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.qingjiabiao.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.biaoti = true;
          ...
          if(o=='qingjiabiao'){
            this.ruleForm.qingjiabiao = obj[o];
	    this.ro.qingjiabiao = true;
            continue;
          }
          if(o=='fabushijian'){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `qingjiabiaoge/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.qingjiabiaogeCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    qingjiabiaoUploadChange(fileUrls) {
	this.ruleForm.qingjiabiao = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{

<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="请假标题" prop="qingjiabiaoti">
          <el-input v-model="ruleForm.qingjiabiaoti" 
              placeholder="请假标题" clearable  :readonly="ro.qingjiabiaoti"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="请假标题" prop="qingjiabiaoti">
              <el-input v-model="ruleForm.qingjiabiaoti" 
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.tianxieqingjiabiao" label="填写请假表" prop="tianxieqingjiabiao">
          <file-upload
          tip="点击上传填写请假表"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.tianxieqingjiabiao?ruleForm.tianxieqingjiabiao:''"
          @change="tianxieqingjiabiaoUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.tianxieqingjiabiao" label="填写请假表" prop="tianxieqingjiabiao">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.tianxieqingjiabiao.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='qingjiabiaoti'){
            this.ruleForm.qingjiabiaoti = obj[o];
	    this.ro.qingjiabiaoti = true;
            continue;
          }
          if(o=='tianxieqingjiabiao'){
            this.ruleForm.tianxieqingjiabiao = obj[o];
	    this.ro.tianxieqingjiabiao = true;
            continue;
          }
          if(o=='tijiaoshijian'){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `tijiaoqingjiabiao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.tijiaoqingjiabiaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    tianxieqingjiabiaoUploadChange(fileUrls) {
	this.ruleForm.tianxieqingjiabiao = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
